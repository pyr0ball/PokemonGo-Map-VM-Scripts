...
#               nohup proxychains4 python $executible -os  "$location" $dbtype $dbname $dbuser $dbpass $dbhost $dbcon -k $GAPI -H $host -P $port &
	...
	python $executable $auth $username $password -l "$location" -H $host -P $port -dc -st $steps -k $GAPI -L $locale $thread $scandelay $logindelay $loginretry $maxfail $dbtype $dbname $dbuser $dbpass $dbhost $dbcon $proxy $jitter $webhooks $webhookth $nopokes $nogyms $nopstops $gyminfo -v maplog.log &
	if [ "$?" -eq "1" ] ; then
	...
	### ~Beehive Workers~###
	# sleep 2
	# nohup  python $executable -a $auth $beehive $beepass -l "Beehive_location1" -dc -st $steps -H 0.0.0.0 -P 5000 -k $GAPI -L $locale $thread $scandelay $dbtype $dbname $dbuser $dbpass $dbhost $dbcon $proxy -ns
	# sleep 2
	# nohup  python $executable -a $auth $beehive $beepass -l "Beehive_location2" -dc -st $steps -H 0.0.0.0 -P 5000 -k $GAPI -L $locale $thread $scandelay $dbtype $dbname $dbuser $dbpass $dbhost $dbcon $proxy -ns
	# sleep 2
	# nohup  python $executable -a $auth $beehive $beepass -l "Beehive_location3" -dc -st $steps -H 0.0.0.0 -P 5000 -k $GAPI -L $locale $thread $scandelay $dbtype $dbname $dbuser $dbpass $dbhost $dbcon $proxy -ns
#                sleep 14400
#        done
